...
- **技术栈**：Go 控制面 + C 数据面
- **核心能力**：多路径 + 乱序重组 + 重叠采样 + FEC + Orchestrator 自适应调度
- **语言分工**：Go 负责路径调度与 BBR v3，C 负责包拆分与 FEC 计算（AVX-512 优化）
- **极端环境生存**：WebRTC DataChannel（伪装视频会议）+ ICMP Tunnel（伪装 Ping）+ DNS Tunnel（伪装域名解析）
...
├─────────────────────────────────────────────────────────┤
│ 传输层（五种协议并存）                                    │
│   ├─ QUIC/UDP (Level 0 — 性能之王)                       │
│   ├─ WebRTC DataChannel (Level 1 — 伪装视频会议)         │
│   ├─ WSS/TCP (Level 2 — 隐蔽防线)                       │
│   ├─ ICMP Tunnel (Level 3 — 伪装 Ping)                  │
│   └─ DNS Tunnel (Level 3 — 求生通道)                     │
└─────────────────────────────────────────────────────────┘
```
