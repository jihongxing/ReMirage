...
| 流量整形与特征隐匿 | NPM / B-DNA / Jitter-Lite / VPC 协同工作，在包长、时序、画像层面降低可识别性 | 1. 配置主源、加载路径、编译路径、运行时挂载全部存在。 2. 关键 `.c` 文件可真实编译。 3. 若要宣称 DPI/ML 对抗效果，必须有独立实验或基准证据 | 协议主源和运行时锚点已建立；OpenCloudOS 目标节点已完成 eBPF verifier load smoke 与 runtime attach smoke；M13 真实 baseline 已完成 `firefox-linux` 与 `chrome-win` 两族，但缺 `chrome-macos`，因此为 `M13-degraded`；M15 degraded 分类器复验已完成，RandomForest 在握手、包长、时序、联合四个检测面均为 `AUC=1.0/F1=1.0/Accuracy=1.0`，证明当前 ReMirage 参考样本仍高度可分，未达到风险下降或能力升级条件。后续 calibrated reference remediation run 将 AUC 降至 C1=0.5293、C2=0.4294、C3=0.3920、C4=0.3931，证明 baseline-driven 校准方向有效，但该结果仍是校准模拟参考，不是真实 ReMirage 流量证据。证据不足以升级为“已实现（限定表述）”，必须维持 `部分实现`。差距：缺少 `chrome-macos` 原生 baseline、缺少真实 ReMirage pcap 派生样本，且需要把校准逻辑落到真实数据面/样本提取链路 | `部分实现` | `docs/protocols/source-of-truth.md`、`docs/protocols/npm.md`、`bdna.md`、`jitter-lite.md`、`vpc.md`、`mirage-gateway/pkg/ebpf/bpf_compile_test.go`、`mirage-gateway/scripts/test-ebpf-compile.sh`、`mirage-gateway/scripts/smoke-ebpf-load.sh`、`mirage-gateway/scripts/smoke-ebpf-runtime-attach.sh`、`docs/reports/m13-real-baseline-degraded.md`、`docs/reports/m15-degraded-classifier-results.md`、`docs/reports/stealth-experiment-results.md`、`docs/reports/stealth-experiment-plan.md`、`docs/reports/stealth-claims-boundary.md`、`artifacts/dpi-audit/`、`deploy/scripts/drill-m6-experiment.sh`、`docs/reports/phase4-evidence-audit.md` |
