...
| 包长分布改变效果实验结果 | ❌ 不存在 | 无 |
| 时序扰动效果实验结果 | ❌ 不存在 | 无 |
| M13 真实 baseline | ⚠️ 降级完成 | `docs/reports/m13-real-baseline-degraded.md`，缺 `chrome-macos` |
| M15 degraded 分类器复验 | ❌ 未达标 | `docs/reports/m15-degraded-classifier-results.md`，四项 AUC 全 1.0 |
...
| 编号 | 不允许表述 | 缺失证据 | 解除条件 |
|------|------------|----------|----------|
| D-1 | DPI/ML 对抗效果达到 N 分 | M15 degraded 复验证明当前样本仍高度可分，四项 AUC 全 1.0 | M13-full + M15 AUC 达标 |
| D-2 | 隐匿效果已显著领先同类产品 | 无对比实验结果 | 需独立对比实验 |
| D-3 | 可抵抗生产级 DPI/ML 系统 | 无真实对抗网络实验 | 需生产环境观测数据 |
| D-4 | 流量特征与真实 Chrome 浏览器无法区分 | M15 degraded 握手分类 AUC=1.0，高可区分性风险仍存在 | M13-full + 握手维度 AUC 达标 |
| D-5 | 包长分布已完全拟态目标流量 | M15 degraded 包长分类 AUC=1.0，高可区分性风险仍存在 | M13-full + 包长维度 AUC 达标 |
| D-6 | 时序特征已无法被 ML 分类器识别 | M15 degraded 时序分类 AUC=1.0，高可区分性风险仍存在 | M13-full + 时序维度 AUC 达标 |
...
> **当前状态：M13 degraded baseline 已采集两族；M15 degraded 分类器复验已完成但未达标。**
>
> `chrome-macos` 缺失，且 M15 degraded 四个检测面 AUC 全为 1.0，因此隐匿能力不得升级。

### 回写项

| 检测面 | 当前不允许表述 | 实验完成后判定 |
|--------|---------------|---------------|
| 握手指纹 | D-4: 流量特征与真实 Chrome 浏览器无法区分 | 维持不允许；M15 degraded C1 AUC=1.0 |
| 包长分布 | D-5: 包长分布已完全拟态目标流量 | 维持不允许；M15 degraded C2 AUC=1.0 |
| 时序分布 | D-6: 时序特征已无法被 ML 分类器识别 | 维持不允许；M15 degraded C3 AUC=1.0 |
| 综合分类 | D-1: DPI/ML 对抗效果达到 N 分 | 维持不允许；M15 degraded C4 AUC=1.0 |
| 综合评分 | D-7: 10 分隐匿效果 | 维持不允许；M13/M15 均未达到升级条件 |
...
- M15 degraded 报告：`docs/reports/m15-degraded-classifier-results.md`
