...
| 隐匿实验结果 | M13/M15 隐匿实验执行、降级边界和分类器结果产出 | `python3 artifacts/dpi-audit/classifier/build-m15-degraded-features.py --baseline-root artifacts/dpi-audit/baseline --simulation-metadata artifacts/dpi-audit/simulation-metadata.json --output artifacts/dpi-audit/classifier/features-m15-degraded.csv --metadata-output artifacts/dpi-audit/classifier/m15-degraded-metadata.json && python3 artifacts/dpi-audit/classifier/train-classifier.py -i artifacts/dpi-audit/classifier/features-m15-degraded.csv -o artifacts/dpi-audit/classifier/results-m15-degraded.json` | 脚本可运行并写出 degraded metadata/results；若 AUC 未达标，必须记录为不升级 | `P1` | `docs/reports/stealth-experiment-results.md`、`docs/reports/m13-real-baseline-degraded.md`、`docs/reports/m15-degraded-classifier-results.md` + `artifacts/dpi-audit/` | `Gateway Runtime` |
...
| 隐匿实验结果 | 2026-04-28 | Gateway Runtime | DEGRADED-HIGH-RISK | `docs/reports/m13-real-baseline-degraded.md`、`docs/reports/m15-degraded-classifier-results.md` | M13 两族真实 baseline 完成，缺 `chrome-macos`；M15 degraded RandomForest 四项 AUC/F1/Accuracy 全 1.0，不允许能力升级 |
