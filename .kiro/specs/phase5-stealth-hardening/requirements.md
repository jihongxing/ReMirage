...
- 1.1 在 Linux 环境采集真实 Chrome（最新稳定版）访问主流 HTTPS 站点的流量，提取握手指纹（TCP SYN 字段 + TLS ClientHello extension 列表 + JA4）、包长分布（前 N 包长度/方向/上下行比例/熵值）、IAT 分布（均值/标准差/P50/P95/P99/burst 结构）
- 1.2 采集至少 3 个目标画像族的对照数据：Chrome-Win、Chrome-macOS、Firefox-Win，每族至少 100 条连接
- 1.3 采集结果替换 `artifacts/dpi-audit/` 中的模拟 pcapng 文件，保留模拟数据为 `*.simulated.pcapng` 后缀备份
- 1.4 更新 `simulation-metadata.json` 标注真实采集环境（内核版本、浏览器版本、网络条件）

### 2. B-DNA 指纹动态化

- 2.1 `bdna.c` 从 per-global `active_profile_map` 改为 per-connection 画像选择：每条新连接根据 `conn_key` 查 `conn_profile_map` 获取 profile_id，再从 `fingerprint_map` 取对应模板
- 2.2 Go 控制面在连接建立时，从画像库中按权重随机选择一个画像族（Chrome/Firefox/Safari/Edge），将 profile_id 写入 `conn_profile_map`
- 2.3 权重配置可通过 `gateway.yaml` 调整，默认权重按全球浏览器市场份额分配
- 2.4 确保同一连接生命周期内画像不变（TCP SYN 重写和后续 QUIC/TLS 参数使用同一 profile）
- 2.5 画像库数据（`fingerprints.yaml` / `profile-registry.v1.json`）中的值必须与真实采集的对照基线一致，发现偏差时更新画像库
...
- 6.4 单维度 AUC 目标 < 0.75，联合 AUC 目标 < 0.85（作为设计目标，实际值由实验决定，不达标不伪造）
...
- 7.3 升级条件：需求 2/3/4 实现完成 + 需求 1 真实基线采集完成 + 需求 6 分类器实验单维 AUC 均 < 0.9 + 联合 AUC < 0.9
