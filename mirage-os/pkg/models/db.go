...
type BillingLog struct {
	...
	GatewayID      string    `gorm:"index;size:32;not null" json:"gateway_id"`
	UserID         string    `gorm:"index;size:64" json:"user_id"`
	...
	LogType        string    `gorm:"index;size:16;default:'traffic'" json:"log_type"` // traffic/deposit/purchase/refund
...
type QuotaPurchase struct {
	ID          uint64    `gorm:"primaryKey;autoIncrement" json:"id"`
	UserID      string    `gorm:"index;size:64;not null" json:"user_id"`
	PackageType string    `gorm:"size:16;not null" json:"package_type"` // 10GB/50GB/100GB/500GB/1TB
