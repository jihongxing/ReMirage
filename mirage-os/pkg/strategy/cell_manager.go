...
// 配置常量
const (
	IncubationDuration = 1 * time.Hour  // 潜伏期时长
	CalibrationSamples = 100            // 校准期采样数
	ScaleOutThreshold  = 0.8            // 容量阈值 80%
	ThreatLevelCritical = 3             // 威胁等级临界值
)
...
func NewCellScheduler(db *gorm.DB) *CellScheduler {
...
func (s *CellScheduler) promoteToCalibration(gw *models.Gateway) error {
	...
	if err := s.db.Model(gw).Updates(map[string]interface{}{
		"phase": PhaseCalibration,
	}).Error; err != nil {
		return err
	}
	
	// TODO: 通知 Gateway 开始网络质量测量
...
func (s *CellScheduler) RegisterGateway(gatewayID, cellID, ipAddress string) error {
	...
	// TODO: 通知 Gateway 开始 VPC 噪声注入
	
	return nil
}

// UpdateNetworkQuality 更新网络质量（校准期上报）
func (s *CellScheduler) UpdateNetworkQuality(gatewayID string, rtt int, packetLoss float64) error {
	// 计算网络质量分数 (0-100)
	// 公式: Quality = 100 - (RTT权重 × RTT归一化) - (丢包权重 × 丢包率)
	rttScore := float64(rtt) / 1000.0 // 假设 1000ms 为最差
	if rttScore > 1.0 {
		rttScore = 1.0
	}
...
func (s *CellScheduler) GetShadowPoolStatus() map[string]interface{} {
	s.mu.RLock()
	defer s.mu.RUnlock()
